...
For the following examples we’ll use a line representing a route around
Aotearoa.
...
to add controls to the map for animating along the route.
...
You can add a customisable speed control. By default, there are 3 speed
options: 0.5x, 1x, and 2x. You can change the speed options by passing
custom values and labels to the
[`add_animation_controls()`](https://epi-interactive-ltd.github.io/toro/reference/add_animation_controls.md)
function.
...
this purpose, or you can add your own custom image. To use a custom
image, use
[`add_image()`](https://epi-interactive-ltd.github.io/toro/reference/add_image.md)
to add the image and then pass the `settings.animatingIcon` parameter to
[`add_route()`](https://epi-interactive-ltd.github.io/toro/reference/add_route.md)
to specify the icon you want to animate along the route.
...
You can also change the styles of the different parts of the route using
the `animatedRouteStyle`, `unanimatedRouteStyle`, and
`visitedRouteStyle` settings.
